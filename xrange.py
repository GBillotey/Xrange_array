...
Implementation details:
    ...
    The unary operations implemented are :
        as numpy unfunc : abs, sqrt, square, conj, log
...
class Xrange_polynomial(np.lib.mixins.NDArrayOperatorsMixin):
    ...
    def  __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        for x in inputs:
            ...
            if isinstance(x, Xrange_polynomial):
                ...
                raise NotImplementedError("Xrange_polynomial view not handled "
                    "for this type {}, supported types : {}.".format(type(x),
    ...
    def _mul(inputs, cutdeg, out=None):
        ...
        if len(op0) > len(op1): # TODO: is it really needed ???   
            op0, op1 = op1, op0
        l0 = len(op0)
        l1 = len(op1)
        ...
        for x in inputs:
            ...
            if isinstance(x, Xrange_SA):
                ...
                if isinstance(x, Xrange_polynomial):
                    ...
                    raise NotImplementedError(
                        "Xrange_SA view not handled for this type {}"
                        ", supported types : {}.".format(type(x),
                        Xrange_array._HANDLED_TYPES + (Xrange_polynomial,
                                                       Xrange_SA)))
