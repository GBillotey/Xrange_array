...
Reference:
    ...
    def fill(self, val):
        ...
        if np.isscalar(val):
            ...
            raise ValueError("Invalid input to Xrange_array.fill, "
                    "expected a numpy scalar or a Xrange_array of null shape")
    ...
    def _need_renorm(val):
        ...
        if val.dtype == np.float32:
            ...
            return np.any((np.abs(((bits >> 23) & 0xff) - 127) > 31) & (val != 0.))
        elif val.dtype == np.float64:
            bits = val.view(np.int64)
            return np.any((np.abs(((bits >> 52) & 0x7ff) - 1023) > 255) & (val != 0.))
    ...
    def _xlog2(val):
        ...
        if val.dtype == np.float32:
            bits = val.view(np.int32)
            return np.where(val == 0., 0, np.abs(((bits >> 23) & 0xff) - 127)).astype(np.int16)
        elif val.dtype == np.float64:
            bits = val.view(np.int64)
            return np.where(val == 0., 0, np.abs(((bits >> 52) & 0x7ff) - 1023)).astype(np.int16)
        elif val.dtype in [np.complex64, np.complex128]:
    ...
    def _exp2_shift(m, shift):
        ...
        if dtype == np.float32:
            ...
            exp = np.clip(((bits >> np.int32(23)) & np.int32(0xff)) + shift,
                          np.int32(0), None)
            return np.copysign(((exp << np.int32(23))  + (bits & np.int32(0x7fffff))
                                ).view(np.float32), m)
        elif dtype == np.float64:
    ...
    def __setitem__(self, key, val):
        ...
        if self.is_complex and not(val.is_complex):
            arr = np.asarray(self)
            arr["mantissa"].real = val._mantissa
            arr["mantissa"].imag = 0.
            arr["exp"] = val._exp
            return
        if not(self.is_complex) and val.is_complex: 
